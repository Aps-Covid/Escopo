...
public class DB_Conn {
    ...
    public static void initializeDatabase() {
        ...
        try (Connection conn = getConnection(); Statement stmt = conn.createStatement()) {
            String createCovidData = "CREATE TABLE IF NOT EXISTS covid_data (" +
            ...
            String createTypeDeaths = "CREATE TABLE IF NOT EXISTS covid_typedeaths (" +
                    "id INT AUTO_INCREMENT PRIMARY KEY, " +
                    "date DATE, " +
                    "deaths_covid19 INT, " +
    ...
    public static void insertCovidData(JSONObject jsonResponse) {
        try (Connection conn = getConnection(); PreparedStatement stmt = conn.prepareStatement(
                "INSERT INTO covid_data (city, state, confirmed, deaths, estimated_population, confirmed_per_100k, date) " +
                        "VALUES (?, ?, ?, ?, ?, ?, ?)")) {
